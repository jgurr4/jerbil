...
public class TestDatabase extends DatabaseContainer {
  ...
  public TestDatabase(Database database, UserTable user, PlayerTable player, ItemTable item, InventoryTable inventory,
                      TestDbTables tables) {
    super(database, IArrayList.make(TableContainer.make(user, IArrayList.make(user.userId, user.name, user.age)),
                          ...
                          TableContainer.make(inventory, IArrayList.make(inventory.itemId, inventory.playerId))));
